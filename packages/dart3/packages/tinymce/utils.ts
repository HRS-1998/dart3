...
export const getTinymce = (): any => {
  // @ts-ignore
  const root: any = typeof window !== 'undefined' ? window : window.global;
  return root && 'tinymce' in root ? root.tinymce : null;
};

export function getContent(editor: any) {
  if (!editor) {
    return;
  }
...
export function setContent(val: any, editor: any) {
  if (!editor) {
    return;
  }
...
export function resetContent(val: any, editor: any) {
  if (!editor) {
    return;
  }
...
export function setModeDisabled(editor: any, disabled = true) {
  if (!editor) {
    return;
  }
...
// export function getContentStyle(style: any) {
