  ...
  <div class="right-drag">
    ...
    <div class="search-box" v-if="filterable">
      <dart-input
        ...
        :placeholder="filterPlaceholder ? filterPlaceholder : '请搜索'"
        :prefix-icon="Search"
        size="default"
        clearable
      />
    ...
    >
      ...
      >
        <div
          class="top-indicator"
          v-show="showTopIndicator && dragOverIndex === index"
        ></div>
        <div class="field-item-container">
          <div
            :class="[
              'drag-handle',
              { 'not-draggable': !isItemDraggable(item) },
            ]"
          ...
          <div
            ...
            v-if="mergedConfig?.showRightIcon && item.disabled"
          >
...
import type { ListItem, RightPanelConfig, PropsMapType } from './types/index';
...
const defaultConfig: RightPanelConfig = {
  name: 'rightPanel',
...
const { mergedConfig } = useMergeConfig(
  defaultConfig,
  props.config as RightPanelConfig
);
const { dragPreview, createDragPreview, waitForImagesAndReturnPreview } =
  useDragPreview();

const emit = defineEmits(['remove', 'outhandleDrop', 'update:modelValue']);
...
const filteredList = computed(() => {
  ...
  return list.value.filter((item: ListItem) => {
    if (props.filterMethod && typeof props.filterMethod === 'function') {
      // 如果配置了自定义搜索函数，则优先使用
      const result = props.filterMethod(keyword, getRawData(item));
      return result;
    }
...
const handleListDrop = (event: DragEvent) => {
  ...
  if (!data || !name) return;
  // 外部拖入
  if (dragItem.value === null && name === mergedConfig.value.dragOrigin) {
...
const remove = (field: ListItem) => {
  list.value = list.value.filter((item: ListItem) => item.id !== field.id);
  emit('remove', field);
};
</script>
...
.right-drag {
  ...
  --field-hover-color: #409eff; /* 字段hover颜色 */
